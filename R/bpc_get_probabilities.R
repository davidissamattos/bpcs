...
get_probabilities<-function(bpc_object, n=1000){
  ...
  if(model=='bt')
  {
    ...
    #Player 1 is i
    #Player 0 is j
    col_names<-c(bpc_object$call_arg$player1, bpc_object$call_arg$player0)
  ...
  if(model=='btordereffect')
  {
    ...
    #Player 1 is i
    #Player 0 is j
    col_names<-c(bpc_object$call_arg$player1, bpc_object$call_arg$player0, bpc_object$call_arg$z_player1)
    colnames(newdata)<-col_names
    pred<-predict(bpc_object, newdata=newdata, n=n,return_matrix=T)
    mean_pred <- apply(pred, 2, mean)
  ...
  if(model=='btU')
  {
    ...
    #Player 1 is i
    #Player 0 is j
    col_names<-c(bpc_object$call_arg$player1, bpc_object$call_arg$player0, bpc_object$call_arg$cluster)
    colnames(newdata)<-col_names
    pred<-predict(bpc_object, newdata=newdata, n=n,return_matrix=T)
    mean_pred <- apply(pred, 2, mean)
  ...
  if(model=='btgeneralized')
  {
    ...
    col_names<-c(bpc_object$call_arg$player1, bpc_object$call_arg$player0)
    colnames(newdata)<-col_names
    pred<-predict(bpc_object, newdata=newdata, predictors=bpc_object$predictors_df, n=n,return_matrix=T)
    mean_pred <- apply(pred, 2, mean)
