...
#' @param model_type
#' * 'bt' (default) for the Bradley Terry model. Ref: Bradley-Terry 1952,
...
#' bpc(data = tennis_agresti, player0 = 'player0',player1 = 'player1',result_column = 'y',model_type = 'bt', solve_ties = 'none')
bpc <- function(data,
                player0,
                player1,
                player0_score=NULL,
  ...
  if(!is.null(z_player1) & !endsWith(model_type,'ordereffect'))
    stop('Error! If the order effect column is specified you should choose either the btordereffect or the davidsonordereffect model')

  if(!is.null(cluster) & !endsWith(model_type,'U'))
    stop('Error! If the cluster column is specified you should choose either the btU or the davidsonU model to handle the random effects of the cluster')

  if(!is.null(predictors) & !endsWith(model_type,'generalized'))
    stop('Error! If the predictors dataframe is specified you should choose either the generalized models btgeneralized or davidsongeneralized.')

  if(solve_ties!='none' & startsWith(model_type,'davidson'))
    warning('You are calling a variation of the Davidson model but you are handling the ties. Consider switching to a Bradley-Terry model')
  ...
  dropna_cols<-c(player0,player1,player0_score,player1_score,result_column,z_player1)
  ...
  if(solve_ties=='none' & !startsWith(model_type,'davidson') & ties_present==T)
  ...
  if(!is.null(cluster))
  {
    ...
    cluster_lookup_table <- create_index_cluster_lookuptable(d,cluster)
  }
  else{
    cluster_lookup_table<-NULL
  }
  ...
  if(!is.null(predictors)){
    if(!check_predictors_df_contains_all_players(predictors,lookup_table))
      stop('The predictor matrix is mispecified. It should contain all the players from the data frame and only them')
    ...
    predictors_matrix<-create_predictor_matrix_with_player_lookup_table(d=predictors,
    ...
    if(!check_numeric_predictor_matrix(predictors_matrix))
      stop('The predictors are mispecified. Only numeric values are accepted. Booleans are accepted but will be cast into integers')
  ...
  if(model_type=='bt'){
    ...
    stanfit <- rstan::sampling(stanmodels$bt, data = standata, chains=chains, iter=iter, warmup=warmup, refresh=refresh, seed=seed)
    ...
    stanfit <- rstan::sampling(stanmodels$davidson, data = standata, chains=chains, iter=iter, warmup=warmup, refresh=refresh, seed=seed)
  }
    ...
    stanfit <- rstan::sampling(stanmodels$btordereffect, data = standata, chains=chains, iter=iter, warmup=warmup, refresh=refresh, seed=seed)
  }
    ...
    stanfit <- rstan::sampling(stanmodels$davidsonordereffect, data = standata, chains=chains, iter=iter, warmup=warmup, refresh=refresh, seed=seed)
    ...
    stanfit <- rstan::sampling(stanmodels$btgeneralized, data = standata, chains=chains, iter=iter, warmup=warmup, refresh=refresh, seed=seed)
    ...
    stanfit <- rstan::sampling(stanmodels$davidsongeneralized, data = standata, chains=chains, iter=iter, warmup=warmup, refresh=refresh, seed=seed)
  }
    ...
    stanfit <- rstan::sampling(stanmodels$davidsonU, data = standata, chains=chains, iter=iter, warmup=warmup, refresh=refresh, seed=seed)
    ...
    stanfit <- rstan::sampling(stanmodels$btU, data = standata, chains=chains, iter=iter, warmup=warmup, refresh=refresh, seed=seed)
  }
  else
