...
create_index_cluster_lookuptable <-function(d,cluster){
  ...
  cluster_index <- seq(1:length(cluster_names)) #sequential indexing starting with 1
  #Now we have a lookup table to convert the indexes
  cluster_lookup_table <- data.frame(Names=cluster_names, Index=cluster_index)
...
match_player_names_to_lookup_table<-function(d,player0, player1, lookup_table)
{
...
match_cluster_names_to_cluster_lookup_table<-function(d, cluster ,cluster_lookup_table)
{
  cluster_index<-cluster_lookup_table$Index[match(unlist(d[,cluster]), cluster_lookup_table$Names)]
...
create_index <- function(d, player0, player1){
  ...
  #Now we have a lookup table to convert the indexes
  lookup_table <- create_index_lookuptable(d, player0=player0, player1=player1)
...
create_index_predictors_with_lookup_table<-function(d,player, lookup_table)
{
  #https://stackoverflow.com/questions/35636315/replace-values-in-a-dataframe-based-on-lookup-table
  d <- as.data.frame(d)
  player_index <- lookup_table$Index[match(unlist(d[,player]), lookup_table$Names)]
...
create_predictor_matrix_with_player_lookup_table<-function(d,player,predictors_columns,lookup_table){
  d <- as.data.frame(d)
  d <- create_index_predictors_with_lookup_table(d,player, lookup_table)
  d <- dplyr::arrange(d,player_index)
...
create_predictors_lookup_table<-function(predictors_columns){
  predictors_index <- seq(1:length(predictors_columns)) #sequential indexing starting with 1
  #Now we have a lookup table to convert the indexes
  lookup_table <- data.frame(Names=predictors_columns, Index=predictors_index)
  return(lookup_table)
}
...
#' @return A dataframe with the additional columns 'player0_index' and 'player1_index' that contains the indexes
create_index_with_existing_lookup_table<-function(d,player0, player1, lookup_table){
  d<-match_player_names_to_lookup_table(d,
                                        player0=player0,
                                        player1=player1,
                                        lookup_table = lookup_table)
...
#' @param cluster_lookup_table a lookup table for the cluster
#' @return A dataframe with the additional columns 'player0_index' and 'player1_index' that contains the indexes
create_cluster_index_with_existing_lookup_table<-function(d,cluster,cluster_lookup_table){
  d<-match_cluster_names_to_cluster_lookup_table(d, cluster=cluster, cluster_lookup_table = cluster_lookup_table)
  return(d)
}
...
#' E.g  for parameter lambda with 4 players we have (lambda[1], lambda[2], lambda[3], lambda[4]). This function will return (lambda[Biometrika], lambda[CommStat], lambda[JASA], lambda[JRSSB])
#' @param d dataframe
...
replace_parameter_index_with_names<-function(d, column, par, lookup_table, cluster_lookup_table=NULL, predictors_lookup_table=NULL){
  d<-as.data.frame(d)
  ...
  else if(par=='U')
  {
    for(i in 1:nrow(lookup_table)){
      for(j in 1:nrow(cluster_lookup_table)){
        ...
        new_name<-paste(par,'[',lookup_table$Names[i],',', cluster_lookup_table$Names[j],']',sep="")
        for(k in 1:nrow(d)){
          d[k,column]<-gsub(pattern=old_name,replacement=new_name,x=d[k,column],fixed = T)#string as is
        }
      }
    }
  }
  else if(par=='B')
  {
    for(i in 1:nrow(predictors_lookup_table)){
        old_name<-paste(par,'[',i,']',sep="")
        new_name<-paste(par,'[',predictors_lookup_table$Names[i],']',sep="")
        for(j in 1:nrow(d)){
          d[j,column]<-gsub(pattern=old_name,replacement=new_name,x=d[j,column],fixed = T)#string as is
        }
      }
...
#' E.g  for parameter lambda with 4 players: c(lambda[Biometrika], lambda[CommStat], lambda[JASA], lambda[JRSSB])
#' @param par  name of the parameter
#' @param lookup_table lookup table of the players
#' @param cluster_lookup_table a lookup table of the clusters
#' @return a data. frame where we change the names in the variable colum to the corresponding parameter_name from the lookup table
create_array_of_par_names<- function(par, lookup_table, cluster_lookup_table=NULL){
  out<-NULL
  if(par=='lambda'){
  ...
  else if(par=='U')
  {
    ...
    sB<-rep('[',n)
    cB<-rep(']',n)
    out<-paste(U,sB,players_nclusters,',',cluster_nplayer,cB, sep="")
  }
  else{
    out<-par
  }
