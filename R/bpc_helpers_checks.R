...
check_if_there_are_na<-function(d, player0,player1,player0_score=NULL,player1_score=NULL,result_column=NULL)
{
  d<-as.data.frame(d)
...
check_numeric_predictor_matrix<-function(predictor_matrix){
  passed<- all(is.numeric(predictor_matrix)) & all(!is.na(predictor_matrix))
  return(passed)
}
...
#' @return TRUE (correct) or FALSE (with problems)
check_predictors_df_contains_all_players<-function(predictor_df,lookup_table){
