...
print.bpc <- function(x){
  ...
  if(startsWith(x$model_type,'davidson')){
    cat('* Large positive values of the nu parameter indicates a high probability of tie regardless of the abilities of theplayers.\n')
    cat('* Large negative values of the nu parameter indicates a low probability of tie regardless of the abilities of the players.\n')
    cat('* Values of nu close to zero indicate that the probability of tie is more dependable on abilities of the players. If nu=0 the model reduces to the Bradley-Terry model.\n')
  }
  if(endsWith(x$model_type,'ordereffect')){
    cat('* Large positive values of the gm parameter indicate that player 1 has a disadvantage. E.g. in a home effect scenario positive values indicate a home disadvantage.\n')
    cat('* Large negative values of the gm parameter indicate that player 1 has an advantage. E.g. in a home effect scenario negative values indicate a home advantage.\n')
    cat('* Values of gm close to zero indicate that the order effect does not influence the contest. E.g. in a home effect it indicates that player 1 does not have a home advantage nor disadvantage.\n')
  }
  if(endsWith(x$model_type,'U')){
    cat('* The U_std indicates the standard deviation of the normal distribution where the parameters U[player, cluster] are drawn from. Higher values of U_std indicates a higher effect of the cluster in the team abitilies.\n')
    cat('* The U[player, cluster] represents the effect of a particular cluster in a particular team ability.\n')
...
summary.bpc <- function(x){
  ...
  print(knitr::kable(get_probabilities(x)$Table,format = 'simple', digits = 2))
  if(endsWith(x$model_type,'ordereffect')){
    cat('NOTES:\n')
  ...
  rank_of_players<-rank_players(x) %>% dplyr::select(Parameter, MedianRank, MeanRank, StdRank)
  print(knitr::kable(rank_of_players,format = 'simple',digits = 2))
...
#'
predict.bpc <- function(object, newdata, predictors=NULL, n=100, return_matrix=F){
  ...
  if(model_type=='bt')
  {
    ...
    newdata<-create_index_with_existing_lookup_table(d=newdata,
  ...
  if(model_type=='btU')
  {
    ...
    newdata<-create_index_with_existing_lookup_table(d=newdata,
                                                     ...
                                                     player1 = object$call_arg$player1,
                                                     lookup_table = lookup_table)
    newdata<-create_cluster_index_with_existing_lookup_table(d = newdata,
  ...
  if(model_type=='btgeneralized')
  {
    ...
    predictors_matrix<-create_predictor_matrix_with_player_lookup_table(d=predictors,
                                                                        ...
                                                                        lookup_table=lookup_table)
    new_predictors_lookup_table<-create_predictors_lookup_table(predictors_columns)
    if(!dplyr::all_equal(new_predictors_lookup_table, predictors_lookup_table))
      stop('The predictor data frame is specified differently than the predictors data frame used to generate the model. We found different predictors or predictors in different order. The columns should be in the same order and contain only the predictors and the players name')
