...
vignette: >
  %\VignetteIndexEntry{Random effects in the Bradley-Terry model}
...
kable(dplyr::sample_n(optimization_algorithms, size=20),
...
kable(matrix(bm_reduced,ncol = 2))
```
...
df_1<- optimization_algorithms %>%  dplyr::group_by(Benchmark, simNumber) %>% 
...
df_3<-  dplyr::tribble(~algo0_name, ~algo1_name, ~y, ~simNumber, ~Benchmark)
...
for(i in 1:nrow(df_2))
{
  ...
  for(j in 1:nrow(comb)){
    ...
    y <- ifelse(diff_rank<0, 1, 0) # if rank0 is higher than rank1, algo1 wins
    df_3 <- tibble::add_row(df_3,
...
```{r}
m <- bpc(data=df_3, player0 = 'algo0_name', player1 = 'algo1_name', result_column = 'y', cluster = 'Benchmark', model_type = 'btU', priors=list(prior_U_std=10.0))
```
...
mcmc_trace(posterior,pars = c("lambda[1]","lambda[2]","lambda[3]","lambda[4]", "lambda[5]", "lambda[6]", "U_std"), n_warmup=1000)
```
...
names <- m$lookup_table$Names
indexes <-m$lookup_table$Index
