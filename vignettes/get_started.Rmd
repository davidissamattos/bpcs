...
```
From the graph above we see a good mix between the chains. Specially after warmup (grey area)
...
```
In this graph we don't see any divergent iterations between lambda[1] and lambda[2]
...
names <- m1$lookup_table$Names
indexes <-m1$lookup_table$Index
...
kable(prob_table, caption = 'Probabilities of one player beating the other', digits = 2) %>% 
  kable_styling()
```
