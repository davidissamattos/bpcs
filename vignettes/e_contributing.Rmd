...
<!--   - `bpc_publciation.R` Functions for publication ready tables and plots -->
...
<!--     - The goal is to keep the number of models small. If the models are related to an existing one add that functionality to the existing models. -->
<!--     - More models -> longer installation time -->
...
<!--   2. `devtools::document()`. This will update the package and recompile the models -->
<!--   3. Restart the r session -->
<!--   4. Run`devtools::load_all()` again -->
...
Currently we rely on few Github actions from r-lib
